...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        NSDictionary *subAttributes = @{NSFontAttributeName : self.subtitleLabel.font};
        CGSize subtitleSize = [self.titleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.subtitleLabel.font.lineHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:subAttributes context:nil].size;
...
- (void)drawRect:(CGRect)rect {
	...
	CGContextAddRect(c, CGRectMake(5, 0, 8, 4)); // to-do: use dynamic points
	CGContextAddRect(c, CGRectMake(5, 6, 8, 4)); // currently fixed size: 22 x 48pt
	CGContextAddRect(c, CGRectMake(5, 12, 8, 4));
	CGContextAddRect(c, CGRectMake(5, 18, 8, 4));
	CGContextAddRect(c, CGRectMake(5, 24, 8, 4));
	//CGContextAddRect(c, CGRectMake(5, 30, 8, 4));
	
	// the arrow
	CGContextMoveToPoint(c, 0, 28);
	CGContextAddLineToPoint(c, 9, 38);
